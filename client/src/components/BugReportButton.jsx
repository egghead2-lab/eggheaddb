...
export function BugReportButton() {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => { setSubmitted(true); setTimeout(() => { setOpen(false); setSubmitted(false); setDescription(''); setPageName(''); }, 2000); },
  });
        ...
        <div className="fixed bottom-20 right-4 z-50 w-80 bg-white rounded-xl shadow-2xl border border-gray-200 overflow-hidden">
          ...
          {submitted ? (
            <div className="p-6 text-center">
              ...
              <div className="text-sm font-medium text-green-700">Bug reported! Thanks for helping.</div>
            </div>
          ) : (
            <div className="p-4 space-y-3">
              <div>
                <label className="text-xs font-medium text-gray-600 block mb-1">What's the bug?</label>
              ...
              <button onClick={() => mutation.mutate({ description, page_url: location.pathname, page_name: pageName || location.pathname })}
