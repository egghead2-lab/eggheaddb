            ...
            <Section title="Availability" defaultOpen={true}>
              <div className="space-y-2">
                {[
                  ...
                  const a = availability.find(x => x.weekday_id === day.id);
                  const isAvail = !!a;
                  const timeRange = a && (a.time_from || a.time_to) ? `${a.time_from || '—'} – ${a.time_to || '—'}` : null;
                  return (
                    <div key={day.id} className={`flex items-center gap-3 px-3 py-2 rounded ${isAvail ? 'bg-green-50' : 'bg-gray-50 opacity-60'}`}>
                      <span className={`w-3 h-3 rounded-full shrink-0 ${isAvail ? 'bg-green-500' : 'bg-gray-300'}`} />
                      <span className="text-sm font-medium text-gray-700 w-20 shrink-0">{day.name}</span>
                      <span className="text-sm text-gray-600 flex-1">
                        {isAvail ? (
                          <>
                            {timeRange || <span className="text-gray-400">Available</span>}
