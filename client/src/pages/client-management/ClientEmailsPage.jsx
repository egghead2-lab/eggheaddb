...
function getToolConfig(key) {
  ...
  switch (key) {
    ...
    case 'new_professor': return {
      ...
      columns: [linkCol(), { key: 'new_professor_name', label: 'New Lead' }, { key: 'old_professor_name', label: 'Was', render: r => <span className="text-gray-400">{r.old_professor_name || '—'}</span> }, throughEh, ...complianceCols],
      getRecipient: defaultRecipient,
      getMergeData: r => ({ school_name: r.school_name || '', class_name: r.class_name || r.program_nickname || '', new_professor_name: r.new_professor_name || '', old_professor_name: r.old_professor_name || '', through_egghead: r.payment_through_us ? 'Yes' : 'No', vaccination_card_required: r.livescan_required ? 'Yes' : 'No', virtus_required: r.virtus_required ? 'Yes' : 'No', tb_test_required: r.tb_required ? 'Yes' : 'No' }),
    };
